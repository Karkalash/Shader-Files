...
{
	float4 noise1, noise2, noise3, finalNoise;
	float perturb;
	float2 noiseCoords;
	float4 fireColor, alphaColor, tempColor;
	...
	// Create the perturbed and distorted texture sampling coordinates that will be used to sample the fire color texture.
	...
	//Color change based on keyboard input, reduce to 0,1 range.
	tempColor.x = colorChange.x / 255.0f;
	tempColor.y = colorChange.y / 255.0f;
	tempColor.z = colorChange.z / 255.0f;
	tempColor.w = 1.0f;

	//Add value of color change to current color
	fireColor += tempColor;
